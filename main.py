...
"""
    ...
    print(sql_query)
    return sql_query, question



def generate_response(results, initial_question):
    completion = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {
            ...
            },
            {
                "role": "user",
                "content": (
                    f"Given the following database query results from the initial question we asked: {initial_question},"
                    f"please summarize the information in a clear "
                    ...
                    f"""Can you give the response in a table of the results based on the following schema:
                    The tables in the database are as follows:
...
def main():
    ...
    for question in questions:
        ...
        sql_query, initial_question = get_sql_query(question)
        results = run_query(sql_query)
        if "Error" in results:
            print(f"Failed to execute query for question '{question}': {results}")
        else:
            response = generate_response(results, initial_question)
